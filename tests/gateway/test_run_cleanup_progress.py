...
async def test_cleanup_registers_callback_and_deletes_on_success(monkeypatch, tmp_path):
    ...
    # and let the scheduled coroutine run to completion. A chained callback
    # is async — await it, mirroring base.py's isawaitable-gated invocation.
    _res = cb()
    if inspect.isawaitable(_res):
        await _res
...
async def test_cleanup_chains_with_existing_callback(monkeypatch, tmp_path):
    ...
    cb = adapter.pop_post_delivery_callback(session_key)
    assert callable(cb)
    # Chained (pre-existing + cleanup) → async wrapper; await it.
    _res = cb()
    if inspect.isawaitable(_res):
        await _res
    for _ in range(20):
